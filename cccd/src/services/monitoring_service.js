/**
 * Monitoring Service - Complete Error Handling
 * Service để monitoring hệ thống, health checks và metrics với error handling đầy đủ
 */
...
class MonitoringService {
    ...
    getMetrics() {
        try {
            ...
            logger.logError(error, { context: 'monitoring_service.getMetrics' });
            ...
            return {
                ...
                responseTime: { count: 0, average: 0, min: 0, max: 0, p95: 0, p99: 0 },
                system: {
                    cpu: { usage: 0, cores: 0, model: 'Unknown' },
                    memory: { usage: 0, total: 0, free: 0, used: 0 },
                    disk: { available: false, error: error.message }
                }
            };
        }
    ...
    getDetailedStats() {
        try {
            ...
            logger.logError(error, { context: 'monitoring_service.getDetailedStats' });
            // Return safe defaults
            return {
                timestamp: new Date().toISOString(),
                uptime: {
