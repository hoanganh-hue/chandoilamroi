...
program
    ...
    .option('-y, --birth-year <year>', 'Năm sinh (ví dụ: 1990)')
    .option('-q, --quantity <number>', 'Số lượng CCCD cần tạo (1-999999)', '10')
    ...
    .action(async(options) => {
        try {
            ...
            if (options.birthYear) {
                birthYear = parseInt(options.birthYear.trim(), 10);
                if (isNaN(birthYear)) {
                    console.error('❌ Năm sinh phải là số nguyên');
                    process.exit(1);
                }
            }
            ...
            if (quantity < 1 || quantity > 999999) {
                console.error('❌ Số lượng phải từ 1 đến 999999');
                process.exit(1);
            }
...
🔧 generate          Tạo danh sách CCCD
   ...
   -y, --birth-year   Năm sinh (ví dụ: 1990)
   -q, --quantity     Số lượng CCCD cần tạo (1-999999)
...
📝 Ví dụ sử dụng:
   node src/main.js cli generate -p "001,079" -g "Nam" -y 1990 -q 5
